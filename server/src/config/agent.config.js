...
export async function generateApplication(description, aiService, cwd = process.cwd()) {
  try {
    ...
    // Use LangChain's withStructuredOutput
    const structuredModel = aiService.model.withStructuredOutput(ApplicationSchema);

    const application = await structuredModel.invoke(`Create a complete, production-ready application for: ${description}
